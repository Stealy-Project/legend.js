...
class RESTMethods {
  ...
  fetchGuildChannels(guildID) {
    return this.rest.makeRequest('get', Endpoints.Guild(guildID).channels, true).then(data => {
      for(const channel of data) this.client.dataManager.newChannel(channel, channel.guild_id)
    });
  }
      ...
      const send = chan => {
        if (content instanceof Array) {
          ...
          this.rest.makeRequest('post', Endpoints.Channel(chan).messages, true, {
            content, tts, nonce, embed, message_reference,
...
async createTemplate(guild, data) {
  ...
  updateCurrentUser(_data, password) {
    ...
    if(_data.avatar) {
      data.avatar = this.client.resolver.resolveBase64(_data.avatar);
    }
    if (!user.bot) {
  ...
  getGuild(guild) {
    return this.rest.makeRequest('get', `${Endpoints.Guild(guild)}?with_counts=true`, true);
  }
  ...
  fetchUserProfile(user, guild) {
    if (guild instanceof Guild) guild = `?guild_id=${guild.id}`
    else guild = "";
    return this.rest.makeRequest('get', `${Endpoints.User(user).profile}${guild}`, true).then(data =>
  ...
  postExternalAssets(id, _data) {
    const data = {};
    if (_data) {
      data.urls = _data;
    }
    return this.rest.makeRequest('post', Endpoints.App(id).externalAssets, true, data).then(res => res);
  }

  getQuests() {
    return this.rest.makeRequest('get', Endpoints.Quest.base, true).then(res => res);
  }
  ...
  heartbeatQuest(id, streamkey) {
    ...
    return this.rest.makeRequest('post', Endpoints.Quest.quests(id).heartbeat, true, data).then(res => res);
  }

  postActivity(_data) {
    const data = {};
    if(_data.application_id) data.application_id = _data.application_id;
    if(_data.token) data.token = _data.token
    if(_data.duration) data.duration = _data.duration;
    if(_data.share_activity) data.share_activity = _data.share_activity;
    if(_data.exePath) data.exePath = _data.exePath;
    if(_data.session_id) data.session_id = _data.session_id;
    if(_data.media_session_id) data.media_session_id = _data.media_session_id;
    if(_data.token) data.token = _data.token;
    return this.rest.makeRequest('post', Endpoints.Activities.base, true, data).then(res => res);
  }
  ...
  checkPomelo(_data) {
    ...
    return this.rest.makeRequest('post', Endpoints.User('@me').pomeloattempt, true, data).then(res => res);
  }
  ...
  acceptInvite(code, session_id) {
    ...
    return new Promise((resolve, reject) =>
      this.rest.makeRequest('post', Endpoints.Invites(code), true, data, false, false, `eyBsb2NhdGlvbjogJ01hcmtkb3duIExpbmsnIH0=`).then(res => {
        console.log(res)
        const handler = guild => {
          if (guild.id === res.guild_id) {
            resolve(guild);
