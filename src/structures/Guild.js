...
class Guild {
  ...
  setup(data) {
     ...
     */
    if (typeof data.max_members !== 'undefined') this.maximumMembers = data.max_members || 500000;
    ...
    if (typeof data.approximate_member_count !== 'undefined') this.approximateMemberCount = data.approximate_member_count;

    if (typeof data.approximate_presence_count !== 'undefined') this.approximatePresenceCount = data.approximate_presence_count;
  ...
  get suppressRoles() {
    if (this.client.user.bot) return null;
    try {
      return this.client.user.guildSettings.get(this.id).suppressRoles;
    } catch (err) {
      return null;
    }
  }
  ...
  /**
   * Whether to suppress highlight mentions
   * <warn>This is only available when using a user account.</warn>
   * @type {?boolean}
   * @readonly
   */
  get suppressHighlights() {
    if (this.client.user.bot) return null;
    try {
      return this.client.user.guildSettings.get(this.id).suppressHighlights;
    } catch (err) {
      return null;
    }
  }
   ...
   * <warn>This is only available when using a user account.</warn>
   * @param {boolean} allow Whether to suppress everyone
   * @returns {Promise<Guild>}
   */
  disableEveryone(allow) {
    if (allow) return this.client.rest.methods.patchClientUserGuildSettings(this.id, { suppress_everyone: allow });
    else return this.client.rest.methods.patchClientUserGuildSettings(this.id, { suppress_everyone: allow });
  }

  /**
   * Suppresses sheduled events mentions.
   * <warn>This is only available when using a user account.</warn>
   * @param {boolean} allow Whether to suppress scheduled events
   * @returns {Promise<Guild>}
   */
  disableScheduledEvents(allow) {
    if (allow) return this.client.rest.methods.patchClientUserGuildSettings(this.id, { mute_scheduled_events: allow });
    else return this.client.rest.methods.patchClientUserGuildSettings(this.id, { mute_scheduled_events: allow });
  }

  /**
   * Suppresses roles mentions.
   * <warn>This is only available when using a user account.</warn>
   * @param {boolean} allow Whether to suppress roles
   * @returns {Promise<Guild>}
   */
  disableRoles(allow) {
    if (allow) return this.client.rest.methods.patchClientUserGuildSettings(this.id, { suppress_roles: allow });
    else return this.client.rest.methods.patchClientUserGuildSettings(this.id, { suppress_roles: allow });
  }

  /**
   * Mutes this guild.
   * <warn>This is only available when using a user account.</warn>
   * @param {boolean} allow Whether to mute guild
   * @returns {Promise<Guild>}
   */
  mute(allow) {
    if (allow) return this.client.rest.methods.patchClientUserGuildSettings(this.id, { muted: allow });
    else return this.client.rest.methods.patchClientUserGuildSettings(this.id, { muted: allow });
  }
  ...
  /**
   * The `Server Booster` role of the guild
   * @type {Role}
   * @readonly
   */
  ...
  fetchChannels() {
    return this.client.rest.methods.fetchGuildChannels(this);
  }

  fetchRoles() {
    return this.client.rest.methods.fetchGuildRoles(this);
  }
   ...
   *   .catch(console.error);
   */
  fetchMembers(query = '', limit = 0, sleepTime = 2e3, time = 120e3, presences = true, nonce = Snowflake.generate()) {
  ...
  fetchByMemberSafety(timeout = 15_000) {
    return new Promise(resolve => {
      ...
      const handler = (members, guild, chunk) => {
        if (guild.id == this.id && chunk.nonce == nonce) {
          if (members.length > 0) {
            ...
            this.client.ws.send({
              op: Constants.OPCodes.SEARCH_RECENT_MEMBERS,
              d: {
                guild_id: this.id,
                query: '',
                continuation_token: members[0]?.id,
                nonce,
              },
            });
          } else {
